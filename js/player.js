...
async function carregarPeladeirosPublico(){
  ...
  try{
    const rows=await sbFetch('/jogadores_publicos?select=id,nome,apelido,instagram,foto_url,posicao_favorita,modalidade,ativo,data_nascimento&order=apelido.asc');
...
async function jogadorVaiImpl(churrasOpt){
  ...
  try{
    ...
    if(existente){
      ...
      const posicao=posicaoConfirmacaoValida(
        existente.posicao,
        existente.pos,
        G.jogadorLogado?.posicao_favorita,
        G.jogadorLogado?.posicao
      );
      if(!posicao){
        showToast('Defina sua posição no perfil antes de confirmar.');
        return;
      }
  ...
  try{
    ...
    if(existente){
      ...
      p.naoVao.push({
        ...
        jogador_id:G.jogadorLogado?.id||existente.jogador_id||null,
        nome,
        posicao:existente.posicao||existente.pos||G.jogadorLogado?.posicao_favorita||null,
        modalidade:existente.modalidade||null,
        churras:existente.churras||null,
      });
      if(eraConfirmado) await _promoverPrimeiroDaEspera(p);
      queueAdminPeladaNotification('withdraw', p.id, existente.id, {
        previous_status: existente.status || null,
        had_previous_confirmation: ['confirmado', 'espera'].includes(String(existente.status || '')),
      });
    } else {
      ...
      const rows = await sbFetch('/confirmacoes', {
        ...
        body: JSON.stringify({
          pelada_id: p.id, nome,
          jogador_id: G.jogadorLogado?.id || null,
          posicao:G.jogadorLogado?.posicao_favorita||'?', time:'pool', pago:false,
      ...
      p.naoVao.push({
        ...
        jogador_id:G.jogadorLogado?.id||null,
        nome,
        posicao:G.jogadorLogado?.posicao_favorita||null,
        modalidade:resolveModalidadeConfirmacao({ jogador_id:G.jogadorLogado?.id||null, nome }),
      });
      queueAdminPeladaNotification('withdraw', p.id, rows[0].id, {
        previous_status: 'new',
        had_previous_confirmation: false,
      });
    }
...
async function abrirRanking(aba) {
  ...
  if (!G.jogadores || !G.jogadores.length) {
    try {
      const rows = await sbFetch('/jogadores_publicos?select=id,nome,apelido,foto_url,posicao_favorita,modalidade,ativo,data_nascimento&order=apelido.asc');
      G.jogadores = await prepararFotosJogadores(rows || []);
    } catch(e) {}
  }
  await renderRanking();
